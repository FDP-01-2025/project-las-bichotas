...
#include "src/deck.h"       // Clase Deck y struct Card
#include "src/player.h"     // Clase Player
#include "src/persistence.h" // Funciones de persistencia de usuarios y ranking
#include "src/utils.h"      // Funciones utilitarias (mostrar mensajes, colores, etc.)
    ...
    while (true) {
        ...
        if ((points >= 17 && points >= playerPoints) || points > 21) {
            if (points > 21) {
                ...
                displayAchievement("DEALER SE PASO!"); // Logro por dealer pasarse
            } else {
...
int main() {
    ...
    do {
        ...
        switch (option) {
            ...
            default: displayMessage("Opcion invalida.", RED_COLOR); // No debería alcanzarse por el while
        }
